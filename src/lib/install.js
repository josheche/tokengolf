...
export function installHooks() {
  ...
  function upsertHook(event, entry) {
    ...
    const filtered = existing.filter(h =>
      !h._tg &&
      !h.hooks?.some(e => e.command?.includes('tokengolf') || e.command?.includes('session-start.js') || e.command?.includes('session-stop.js') || e.command?.includes('session-end.js') || e.command?.includes('pre-compact.js') || e.command?.includes('post-tool-use.js') || e.command?.includes('user-prompt-submit.js'))
    );
  ...
  upsertHook('SessionEnd', {
    hooks: [{ type: 'command', command: `node ${path.join(HOOKS_DIR, 'session-end.js')}`, timeout: 30 }],
  });

  upsertHook('PreCompact', {
    hooks: [{ type: 'command', command: `node ${path.join(HOOKS_DIR, 'pre-compact.js')}`, timeout: 5 }],
  });

  // Install statusLine (non-destructive: wrap existing if present)
  try { fs.chmodSync(STATUSLINE_PATH, 0o755); } catch {}
  ...
  if (!alreadyOurs && existingCmd) {
    ...
    settings.statusLine = { type: 'command', command: WRAPPER_PATH, padding: 2 };
    console.log('  ✓ statusLine       → wrapped your existing statusline + tokengolf HUD');
  } else if (!alreadyOurs) {
    settings.statusLine = { type: 'command', command: STATUSLINE_PATH, padding: 2 };
